...
### 13. `debuggingChallenge.ts` - Комплексный тест на чтение кода и дебаггинг
**Проблема**: Сложная система с множественными проблемами для анализа.
```typescript
// Содержит проблемы:
// - Race conditions в Promise.all
// - Потенциальные утечки памяти в кэше
// - Проблемы с обработкой ошибок
// - Сложные цепочки асинхронных вызовов
```
**Что нужно проверить**: 
- Понимание потока выполнения программы
- Анализ асинхронных операций и промисов
- Выявление race conditions и утечек памяти
- Навыки статического анализа кода
- **См. подробные инструкции в `DEBUGGING_CHALLENGE.md`**

---

### 14. `debuggingScenarios.ts` - Практические сценарии для дебаггинга
**Проблема**: Коллекция типичных проблем в JavaScript/TypeScript.
```typescript
// Включает сценарии:
// - Promise.all error handling
// - Cache race conditions  
// - Memory leaks в event listeners
// - Проблемы с async/await в циклах
// - Потеря контекста this
// - Замыкания в циклах
```
**Что нужно исправить**:
- Использовать Promise.allSettled для устойчивости к ошибкам
- Реализовать proper cleanup для предотвращения утечек
- Исправить проблемы с контекстом и замыканиями
